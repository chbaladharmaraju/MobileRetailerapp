  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }} className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-bold text-white tracking-tight">Customer Database</h2>
          <p className="text-xs text-ag-text-muted mt-1">View all customers and their purchase history across all modules.</p>
        </div>
          ...
          <table className="ag-table w-full">
            ...
            <tbody>
              {loading && customers.length === 0 ? (
                <tr><td colSpan="4" className="text-center py-20 text-ag-text-dim text-sm">
                ...
                <tr>
                  <td colSpan="4" className="text-center py-20">
                    <HiOutlineUserGroup className="w-12 h-12 text-white/10 mx-auto mb-4" />
                    <p className="text-ag-text-dim text-sm">No customers found.</p>
                  </td>
                </tr>
              ) : customers.map((c, i) => (
                <motion.tr key={c.id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
                  <td>
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-ag-text-muted mt-0.5">{c.phone}</span>
                    </div>
                  </td>
                  <td>
                    <div className="flex items-center justify-center gap-2">
                       ...
                       {(!c._count || (c._count.sales === 0 && c._count.repairs === 0 && c._count.secondHandIntakes === 0)) && <span className="text-[10px] text-ag-text-dim">No history</span>}
                    </div>
                  </td>
                  <td className="text-right">
                    {parseFloat(c.creditBalance) > 0 ? (
                      ...
                      <span className="text-ag-text-dim text-xs">Clear</span>
                    )}
                  </td>
                  <td className="text-right">
                    <div className="flex justify-end gap-2">
                      <Link to={`/customers/${c.id}/ledger`} className="ag-btn bg-white/5 hover:bg-white/10 text-white text-[10px] py-1.5 px-3 h-auto">
                        Ledger
                      </Link>
                    </div>
                  </td>
