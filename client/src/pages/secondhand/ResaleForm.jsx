    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: "easeOut" }} className="max-w-6xl mx-auto pb-12">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Recording resale for {intake.brand} {intake.model}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <div className="glass-card p-6 border-white/10 relative overflow-hidden">
            ...
            <div className="space-y-5 relative z-10">
              <div>
                <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-1">Brand & Model</p>
                <p className="text-base font-bold text-white">{intake.brand} {intake.model}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-1">IMEI</p>
                  <p className="text-sm font-mono text-slate-300">{intake.imei}</p>
                </div>
                <div>
                  <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-1">Condition</p>
              ...
              <div className="pt-5 border-t border-white/5">
                <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-1">Initial Buy Price</p>
                <p className="text-2xl font-black text-white tracking-tight">₹{parseFloat(intake.buyingPrice).toLocaleString('en-IN')}</p>
              </div>
            </div>
          </div>
          ...
          {intake.notes && (
            <div className="glass-card p-5 border-white/5 bg-white/[0.02]">
               <h3 className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-2">Original Intake Notes</h3>
        ...
        <div className="lg:col-span-2">
          <div className="glass-card p-8 sm:p-10 border-blue-500/10 shadow-[0_8px_32px_rgba(0,0,0,0.3)] relative overflow-hidden">
            <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/5 rounded-full blur-[80px] pointer-events-none" />
            
            <h2 className="text-sm font-bold text-white uppercase tracking-widest mb-8 flex items-center gap-2 border-b border-white/5 pb-4 relative z-10">
            ...
            <form onSubmit={handleSubmit} className="relative z-10 space-y-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest ml-1 block">Buyer Full Name</label>
                ...
                <div className="space-y-2">
                  <label className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest ml-1 block">Phone Number</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest ml-1 block">Final Selling Price (₹)</label>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 font-bold">₹</span>
                    <input type="number" min={0} value={form.sellingPrice} onChange={(e) => setForm({ ...form, sellingPrice: e.target.value })} placeholder="0.00" className="ag-input pl-10 text-xl font-bold text-white focus:border-blue-500" required />
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest ml-1 block">Payment Method</label>
              ...
              {/* Profit Indicator */}
              <div className="bg-black/40 rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 border border-white/5 relative overflow-hidden group">
                ...
                <div className="text-center sm:text-left relative z-10">
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1.5">Net Profit Projection</p>
                ...
                <div className="flex items-center gap-4 relative z-10">
                   <div className={`p-4 rounded-2xl border ${calculateProfit() > 0 ? 'bg-green-500/10 border-green-500/20 text-green-400 shadow-[0_0_15px_rgba(74,222,128,0.2)]' : 'bg-white/5 border-white/10 text-slate-500'}`}>
                     <HiOutlineCheckCircle className="w-8 h-8" />
                   </div>
                   <p className="text-[11px] text-slate-400 max-w-[140px] leading-relaxed hidden md:block">
